  ...
  if (model == 0)
    {
      ...
      if (m_mobilityStack.empty ())
      ...
      else
        {
         ...
         // std::cout <<"node " << hierarchical << std::endl;
          NS_LOG_DEBUG ("node="<<object<<", mob="<<hierarchical);
        }
    }


  //m_control->CreateTxtNote(12);

  model->SetAppControl(m_control);
  Vector position = m_position->GetNext ();
  model->SetPosition (position);
  model->SetSharedId (sharedId);
...
void
MobilityHelper::DoSetAppControl (Ptr<ApplicationControl> control)
{
  //control->WriteMessage();
  m_control = control;

}

Ptr<ApplicationControl>
MobilityHelper::DoGetAppControl (void) const
{
  return m_control;
}
