...
public:
  ...
  */
  ...
  void SetShadowValue (const int index, const double shadowValue);
  double GetShadowValue (const int index);
  void SetSharedId(int id);
  int GetSharedId (void);
  /**
   ...
   * \return the distance between the two objects. Unit is meters.
   */
  double GetPreDistanceFrom (Ptr<const MobilityModel> position) const;
    /**
     * \return the previous position.
     *
     ...
     */
  virtual Vector DoGetPrePosition (void) const;
  /**
  ...
  void SetAppControl (Ptr<ApplicationControl> control);

  Ptr<ApplicationControl> GetAppControl (void) const;
  ...
  Vector td_adhoc_ME_city_coordinate_discrete (const double lvf_vehicle_x, const double lvf_vehicle_y) const;

  int td_adhoc_ME_city_los_status_judge (const int lvc_vehicle1_grid_x, const int lvc_vehicle1_grid_y, const int lvc_vehicle2_grid_x, const int lvc_vehicle2_grid_y) const;
...
private:
  ...
  std::vector <double> m_shadowValue;
  /**
   * \return the current position.
   *
   * Concrete subclasses of this base class must 
  ...
  Ptr<ApplicationControl> m_appControl;
