...
NS_OBJECT_ENSURE_REGISTERED (LinePositionAllocator);

TypeId
LinePositionAllocator::GetTypeId (void)
{
  
  static TypeId tid = TypeId ("ns3::LinePositionAllocator")
  
    .SetParent<PositionAllocator> ()
    .SetGroupName ("Mobility")
    .AddConstructor<LinePositionAllocator> ();
/*
...
LinePositionAllocator::LinePositionAllocator ()
{
  m_lrv = CreateObject<UniformRandomVariable> ();
  nrv = CreateObject<NormalRandomVariable> ();
  nrv->SetAttribute ("Mean", DoubleValue (0.0));
  nrv->SetAttribute ("Variance", DoubleValue (225.0));
  nrv->SetAttribute ("Bound", DoubleValue (30.0));
  preX = m_lrv->GetValue (0, 200);
...
void
LinePositionAllocator::SetDirection (int direction, double location)
{
...
void
LinePositionAllocator::SetMaxX (double x)
{
  maxX = x;
}

void
LinePositionAllocator::SetMaxY (double y)
{
  maxY = y;
}

Vector
LinePositionAllocator::GetNext (void) const
{
  double step;
  //double x,y;
  step = nrv->GetValue();
  step += 2.5 * 30;  
  //step = m_lrv->GetValue (0, 100);

  switch(m_direction){
  case 0:
        preX = fmod (step + preX, maxX);
        //y = preY;
        break;
  case 1:
        preX = fmod (step + preX, maxX);
  ...
  //preX = x;
  //preY = y;
  NS_LOG_DEBUG ("Disc position x=" << preX << ", y=" << preY);
  return Vector (preX, preY, 0.0);
}

int64_t
LinePositionAllocator::AssignStreams (int64_t stream)
{
  m_lrv->SetStream (stream);
  return 1;
}
