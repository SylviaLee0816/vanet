...
WaveNetDevice::SendX (Ptr<Packet> packet, const Address & dest, uint32_t protocol, const TxInfo & txInfo)
{
  ...
  //Simulator::Schedule (now+Seconds (0.1), &WaveNetDevice::CancelPacket, this, txInfo.channelNumber, packet, AC_BE);
  return true;
}
...
void
WaveNetDevice::CancelPacket (uint32_t channelNumber,Ptr<Packet> packet, enum AcIndex ac)
{

 // std::cout << "system out error  "<<std::endl;
  Ptr<OcbWifiMac> mac = GetMac (channelNumber);
  mac->CancelPacket(packet,ac);
}

void
WaveNetDevice::SetCchLock ()
{
  m_CchLock = 1;
}

void
WaveNetDevice::SetCchUnlock ()
{
  m_CchLock = 0;
}
