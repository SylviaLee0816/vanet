...
YansWifiChannelHelper
YansWifiChannelHelper::Mobility (const int model)
{
  YansWifiChannelHelper helper;
  helper.SetPropagationDelay ("ns3::ConstantSpeedPropagationDelayModel");
  
  if (model == 0) {
    helper.AddPropagationLoss ("ns3::HighwayPropagationLossModel");
  } else if (model == 1) {
    helper.AddPropagationLoss ("ns3::CityPropagationLossModel");
  }
  return helper;
}
