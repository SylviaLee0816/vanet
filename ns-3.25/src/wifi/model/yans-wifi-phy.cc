                                            ...
                                            Time rxDuration)
{
  ...
  //std::cout << "get the packet notify" << std::endl;
  AmpduTag ampduTag;
                                 ...
                                 Ptr<InterferenceHelper::Event> event)
{
  ...
  //std::cout << "snr(dB)=" << RatioToDb (snrPer.snr) << ", per=" << snrPer.per << std::endl;
...
#ifdef DEBUG_LOG_RECEIVEOK
           Ptr<MobilityModel> senderMobility = GetMobility ()->GetObject<MobilityModel> ();
           senderMobility->GetAppControl()->WriteMessage(packet->GetUid(),senderMobility->GetPosition().x,senderMobility->GetPosition().y);
#endif
        }
      else
        {
