...
/**
position allocator for each line 
**/
class LinePositionAllocator : public PositionAllocator
{
public:
  /**
   * Register this type with the TypeId system.
   ...
   */
  static TypeId GetTypeId (void);
  LinePositionAllocator ();
  virtual ~LinePositionAllocator ();

  /** 
   * \param the direction for load
   */
  void SetDirection (int direction, double location);
  /** 
   * \param x  the X coordinate of the center of the disc
   */
  ...
  virtual int64_t AssignStreams (int64_t stream);
private:
  Ptr<UniformRandomVariable> m_lrv;  //!< pointer to uniform random variable 
  Ptr<NormalRandomVariable> nrv;
  int m_direction;
  double maxX;
  double maxY;
  mutable double preX;
