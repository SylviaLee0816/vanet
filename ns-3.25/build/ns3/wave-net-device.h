...
public:
  ...
  void CancelTx (uint32_t channelNumber, enum AcIndex ac);
   ...
   /**
   *
   * Lock the Cch Channel Switching 
   */
  void SetCchLock ();

    /**
     * Unlock the Cch Channel Schedule
     */
   ...
   * \param  packet
   * \param ac the specific access category
   * cancel the packet transmission 
   */
   
   void CancelPacket (uint32_t channelNumber,Ptr<Packet> packet, enum AcIndex ac);
  /**
   * \param packet packet sent from above down to Network Device
    * \param dest mac address of the destination (already resolved)
