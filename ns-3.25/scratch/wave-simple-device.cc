...
WaveNetDeviceExample::CreateHighwayWaveNodes (void)
{
  ...
  //control->CreateTxtNote (11, 3.5, 4.5);
       ...
       for (uint32_t i = 0 ;i < nNodes; ++i)
        {
          ...
          std::cout << model->GetPosition().x << "    "<< model->GetPosition().y << std::endl;
        }
  control->SetNodeContainer(total);
  control->CreateTxtNote (11, 3.5, 4.5);
  ...
  wavePhy.Set("TxPowerEnd",DoubleValue(23.0));
  wavePhy.Set("TxPowerStart",DoubleValue(23.0));
  wavePhy.Set("TxGain",DoubleValue(1.5));
  wavePhy.Set("RxGain",DoubleValue(1.5));
